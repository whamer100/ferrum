...
namespace Utils {
    ...
    // duplicate two streambufs (i.e. stdout and a log file
    // example usage:
    // Utils::DualStreamBuf dual_buf(std::cout.rdbuf(), out.rdbuf());
    // std::cout.rdbuf(&dual_buf); // redirect std::cout to dual buffer
    // ...
    // std::cout.rdbuf(std::cout.rdbuf()); // restore original buffer
    class DualStreamBuf final : public std::streambuf {
    public:
        DualStreamBuf(std::streambuf* src, std::streambuf* dst)
    ...
    private:
        bool do_mirroring = true;
        std::streambuf* src_buf;
        std::streambuf* dst_buf;
    };
